...
const cardStyles = (disabled: boolean) => css({
    flex: '1 1 0',
    display: 'flex',
    flexDirection: 'column',
...
const cardTopStyles = (gradient: string) => css({
    flex: 1,
    background: gradient,
    display: 'flex',
    alignItems: 'center',
...
function SectionCard({ icon, title, description, gradient, cta, onOpen, disabled = false }: SectionCardProps) {
    return (
        <div css={cardStyles(disabled)} onClick={disabled ? undefined : onOpen}>
            ...
            <div css={cardBodyStyles}>
                ...
                <Button variant="ghost" disabled={disabled} style={{ marginTop: 8 }}>{cta}</Button>
            </div>
        </div>
    );
}
