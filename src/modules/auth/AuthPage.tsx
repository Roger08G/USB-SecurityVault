...
const innerStyles = (wide: boolean) => css({
    width: '100%',
...
function describeError(err: unknown): string {
    if (err instanceof VaultError) {
        switch (err.detail.kind) {
            case 'invalid': return 'Credenciales incorrectas.';
            case 'rate_limited': return `Demasiados intentos. Espera ${Math.ceil(err.detail.seconds / 60)} min.`;
            case 'locked': return 'Bóveda bloqueada.';
            case 'not_initialized': return 'La bóveda no existe.';
            case 'already_initialized': return 'La bóveda ya existe.';
            ...
            <div css={innerStyles(phase === 'show-secret')}>
                ...
                {phase === 'show-secret' && otpauth && (
                    <div css={qrWrapStyles}>
                        <p css={helpStyles}>
                            Escanea este QR con la app <b>Contraseñas</b> de tu iPhone para activar el 2FA.
                        </p>
                        ...
                        <button css={toggleStyles} type="button" onClick={() => setStepsOpen((v) => !v)}>
                        ...
                        {stepsOpen && (
                            <div css={stepsStyles}>
                                <div css={stepItemStyles}>
                                    ...
                                    <span>Abre la app <b>Contraseñas</b> (icono de llave, iOS 18+)</span>
                                </div>
                                <div css={stepItemStyles}>
                                    <span css={stepNumStyles}>2</span>
                                    <span>Pulsa <b>+</b> → rellena nombre y usuario</span>
                                </div>
                                <div css={stepItemStyles}>
                                    <span css={stepNumStyles}>3</span>
                                    <span>En <b>Código de verificación</b>, pulsa el icono de QR</span>
                                </div>
                                <div css={stepItemStyles}>
                                    <span css={stepNumStyles}>4</span>
                                    <span>Apunta la cámara <b>desde ahí</b> a este QR</span>
                                </div>
                            </div>
                        )}
                        ...
                        <Button full onClick={() => { setOtpauth(null); setPhase('password'); }}>
                ...
                {phase === 'totp' && (
                    <form css={formStyles} onSubmit={submitTotp}>
                        ...
                        />
                        <div css={errorStyles}>{error}</div>
                        <Button type="submit" full leftIcon={<LuKeyRound />} disabled={busy || code.length < 6}>
