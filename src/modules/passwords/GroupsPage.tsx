...
const cardStyles = (deleteMode: boolean) => css({
	position: 'relative',
...
const cardImageStyles = (idx: number) => css({
	height: 200,
	width: '100%',
...
const cardDescStyles = css({ margin: 0, fontSize: 14, color: theme.color.textMuted, lineHeight: 1.55 });
const cardCountStyles = css({ marginTop: 8, fontSize: 12, color: theme.color.accent, fontFamily: theme.font.mono, letterSpacing: '0.04em' });
		...
		<main css={pageStyles}>
			...
			{loading ? (
				...
				<section css={gridStyles}>
					{groups.map((g, idx) => (
						<article key={g.id} css={cardStyles(deleteMode)} onClick={() => handleCardClick(g)}>
							{deleteMode && (
								<div css={deleteBadgeStyles}><FiTrash2 /></div>
							)}
			...
			{/* ── Create modal ── */}
			<Modal open={creating} onClose={() => { setCreating(false); setBannerDataUrl(null); }} title="Nueva categoría">
				<form css={formStyles} onSubmit={submitCreate}>
					...
					/>
					<Input label="Nombre" value={name} onChange={(e: React.ChangeEvent<HTMLInputElement>) => setName(e.target.value)} autoFocus />
					<Input label="Descripción" value={desc} onChange={(e: React.ChangeEvent<HTMLInputElement>) => setDesc(e.target.value)} />
					<Button type="submit" full>Crear</Button>
				</form>
			</Modal>

			{/* ── Delete confirm modal ── */}
			<Modal open={!!pendingDelete} onClose={() => setPendingDelete(null)} title="Eliminar categoría">
				<div css={confirmBodyStyles}>
					...
					<p css={confirmTextStyles}>
						...
						Se borrarán <b>todas las cuentas</b> que contiene. Esta acción no se puede deshacer.
					</p>
