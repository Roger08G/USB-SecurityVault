...
const variants: Record<Variant, ReturnType<typeof css>> = {
	primary: css({
		...
		'&:hover:not(:disabled)': { boxShadow: theme.shadow.glowStrong, transform: 'translateY(-1px)' }
	}),
	ghost: css({
		background: 'transparent',
		color: theme.color.text,
		borderColor: theme.color.border,
		'&:hover:not(:disabled)': { borderColor: theme.color.borderStrong, background: theme.color.bgGlass }
	}),
	danger: css({
		background: 'transparent',
...
export function Button({ variant = 'primary', size = 'md', leftIcon, rightIcon, full, children, ...rest }: ButtonProps) {
	return (
		<button
			{...rest}
